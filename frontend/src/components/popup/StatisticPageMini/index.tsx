		...
		<div className={s.wrapper}>
			<Col width="1164px">
				...
				<CourseForBlogger
					...
					title="Курсы английского языка рекламная комп…"
					width_block="1164px"
					width_text="1164px"
				/>
				<Line width="1164px" />
				...
				<Row width="auto" className={s.RowButtonsGraph}>
					<div className={s.extendedLink}>
						<Button width="101px" onClick={() => setGraphChange(1)} className={`${s.extendedButton} ${graphChange === 1 ? s.activeGraphBtn : ''}`} text="Компании" />
						<Button width="68px" onClick={() => setGraphChange(2)} className={`${s.extendedButton}  ${graphChange === 2 ? s.activeGraphBtn : ''}`} text="Цели" />
						<Button width="90px" onClick={() => setGraphChange(3)} className={`${s.extendedButton}  ${graphChange === 3 ? s.activeGraphBtn : ''}`} text="Воронка" />
					</div>
