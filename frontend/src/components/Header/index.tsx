...
const Header: React.FC = () => {
	...
	}, [isProfileOpen, isNotificationOpen, isStatAcc])
	return (
		<>
			{!['/login', '/register', '/choose'].includes(
			...
			{isProfileOpen && (
				<div ref={profilePopUpRef} className="absolute right-8 z-10"> {/* right-[10px] */}
					<ProfilePopUp blogger={user?.isBlogger} />
				</div>
			)}
			
			{isNotificationOpen && (
				<div ref={notificationPopUpRef} className="absolute right-8 z-10">  {/* right-[150px] */}
					<NotificationMenu
			...
			{isStatAcc && (
				<div ref={statAccPopUpRef} className="absolute right-8 z-10"> {/* right-[70px] */}
					<StatusAcc />
				</div>
			)}
		</>
